...
class Spotifys extends Component {
  ...
  componentDidMount() {
    ...
    spotifyWebApi.getUserPlaylists().then(response => {
      ...
      spotifyWebApi
        .searchTracks(
          `${this.props.params.band} ${this.props.params.songTitle}`
        )
        .then(response => {
  ...
  addToPlaylist = value => {
    spotifyWebApi
      .searchTracks(`${this.props.params.band} ${this.props.params.songTitle}`)
      .then(response => {
    ...
    const trackId = getSong => {
      spotifyWebApi.getTrack([getSong]).then(response => {
        spotifyWebApi
          .addTracksToPlaylist(this.state.playlists[this.state.target], [
            response.uri
          ])
          .then(response => {
  ...
  render() {
    ...
    return this.state.spotifyLoggedIn === false ? (
      <div>
        Please login with spotify in the settings to play a sample of this song
      </div>
    ) : (
        ...
        <div>
          ...
          <select onChange={e => this.handleChange(e)}>
            {Object.keys(this.state.playlists).map(name => {
              return (
                <option tar={Object.values(this.state.playlists)}>
                  {name}
                </option>
              );
            })}
